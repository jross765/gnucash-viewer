...
public class DesriptionCellRenderer implements TableCellRenderer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DesriptionCellRenderer.class);
	...
	public Component getTableCellRendererComponent(JTable table, Object value,
		...
		try {
			...
			if ( model instanceof GnuCashTransactionSplitsTableModel ) {
				GnuCashTransactionSplitsTableModel trxSpltModel = (GnuCashTransactionSplitsTableModel) model;
				GnuCashTransactionSplit split = trxSpltModel.getTransactionSplit(row);
				if ( split != null ) {
					if ( split.getTransaction() != null ) {
						try {
							List<? extends GnuCashTransactionSplit> spltList = split.getTransaction().getSplits();
							for ( GnuCashTransactionSplit splt : spltList ) {
								if ( splt != null ) {
									markTokens(renderer, splt.getDescription());
								}
							}
						} catch (Exception e) {
