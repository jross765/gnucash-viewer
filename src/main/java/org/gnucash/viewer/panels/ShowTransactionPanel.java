...
public class ShowTransactionPanel extends JPanel {
	...
	protected JPopupMenu getCellPopupMenu(final int row) {
		...
		final GnuCashTransactionSplit splt = model.getTransactionSplit(row - 1);
		if ( splt != null ) {
	...
	protected void setModel(final SingleTransactionTableModel aModel) {
		...
		spltTab.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setPreferredWidth(SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.DIVIDEND.toString())); //$NON-NLS-1$
		...
		spltTab.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setMaxWidth((int) ( SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.ATM_DEPOSIT.toString()) * FACTOR_COL_WIDTH_ACTION ) ); //$NON-NLS-1$
		// END col widths
		// ---
		...
		if (spltTab == null) {
			spltTab = new JTable() {
				...
				public String getToolTipText(final MouseEvent event) {
					...
					if ( rowIndex > 0 ) {
						GnuCashTransactionSplitsTableModel model = (GnuCashTransactionSplitsTableModel) getModel();
						GnuCashTransactionSplit localSplit = model.getTransactionSplit(rowIndex - 1);
