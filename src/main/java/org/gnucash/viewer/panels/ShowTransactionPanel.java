...
public class ShowTransactionPanel extends JPanel {
	...
	protected void setModel(final SingleTransactionTableModel aModel) {
		...
		trxTab.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setPreferredWidth(SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.DIVIDEND.toString())); //$NON-NLS-1$
		...
		trxTab.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setMaxWidth((int) ( SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.ATM_DEPOSIT.toString()) * FACTOR_COL_WIDTH_ACTION ) ); //$NON-NLS-1$
		// END col widths
		// ---
	}
