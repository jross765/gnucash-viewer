...
public class ShowTransactionPanel extends JPanel {
	...
	protected void setModel(final SingleTransactionTableModel aModel) {
		...
		transactionTable.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setPreferredWidth(SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.DIVIDEND.toString())); //$NON-NLS-1$
		...
		transactionTable.getColumn(Messages_ShowTransactionPanel.getString("ShowTransactionPanel.7")).setMaxWidth((int) ( SwingUtilities.computeStringWidth(metrics, GnuCashTransactionSplit.Action.ATM_DEPOSIT.toString()) * FACTOR_COL_WIDTH_ACTION ) ); //$NON-NLS-1$
		// END col widths
		// ---
	}
	...
	protected Collection<TransactionSplitAction> getSplitActions() {
		LOGGER.info("getSplitActions: ShowTransactionPanel has " + (mySplitActions == null ? "no" : mySplitActions.size()) + " split-actions"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
